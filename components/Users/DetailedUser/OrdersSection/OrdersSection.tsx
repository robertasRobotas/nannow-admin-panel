...
const OrdersSection = ({ user, onBackClick }: OrdersSectionProps) => {
  ...
  const fetchClosedProviderOrders = useCallback(async () => {
    try {
      const collected: OrderType[] = [];
      let startIndex = 0;
      let total = Infinity;
      ...
      while (collected.length < Math.min(total, MAX_SCAN)) {
        ...
        pageSize = result.pageSize ?? items.length ?? 50;
        total = result.total ?? total;
        collected.push(...items.filter(orderMatchesProvider));
        if (items.length === 0) break;
        startIndex += pageSize;
      }
  ...
  }, [orderMatchesProvider, providerId]);

  useEffect(() => {
    fetchClosedProviderOrders();
  }, [fetchClosedProviderOrders]);
