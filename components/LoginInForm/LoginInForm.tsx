...
const LoginForm = () => {
  ...
  const handleAuthResponse = (response: {
    status: number;
    data: { jwt?: string; requires2fa?: boolean; mfaToken?: string };
  }) => {
  ...
  const handleFirebaseLogin = async (provider: "google" | "apple") => {
    ...
    <div className={styles.wrapper}>
      ...
      <form className={styles.card} onSubmit={onSubmit}>
        ...
        {!mfaToken ? (
          <>
            ...
            <div className={styles.ssoRow}>
              <Button
                type="button"
                variant="outline"
                className="flex-1 h-12 text-base font-semibold gap-2.5 [&_svg]:size-6"
          ...
          <>
            <p className={styles.mfaHint}>Enter the 6-digit code from Google Authenticator.</p>
            <div className={styles.field}>
            ...
            <div className={styles.ssoRow}>
              ...
              >
                {isTotpVerifying ? "Verifying…" : "Verify code"}
              </Button>
              <Button
                type="button"
                variant="outline"
                className="flex-1 h-12 text-base font-semibold"
                disabled={isTotpVerifying}
                onClick={() => { setMfaToken(null); setTotpCode(""); setError(false); setAuthErrorMessage(""); }}
              >
                Back
              </Button>
            </div>
