import { useEffect, useRef, useState, type FormEvent } from "react";
import ModalPageTemplate from "@/components/ModalPageTemplate/ModalPageTemplate";
import { backfillMissingCancellationEvents, getOrderEventsList, getOrderEventTypes, regenerateOrderEventTypes } from "@/pages/api/fetch";
import { OrderStatusEnum } from "@/types/OrderStatusEnum";
import styles from "./orderEvents.module.css";

type EventUser = { firstName?: string; lastName?: string; imgUrl?: string };
type EventRow = { id: string; type: string; category?: string; orderId: string; orderPrettyId?: string; orderStatus?: string; occurredAt?: string; actorType?: string; source?: string; data?: Record<string, unknown> | null; provider?: EventUser | null; client?: EventUser | null };
const pretty = (value: string) => value.replaceAll("_", " ").toLowerCase().replace(/(^| )\w/g, (m) => m.toUpperCase());
const date = (value?: string) => value ? new Date(value).toLocaleString("en-GB", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", timeZoneName: "short" }) : "-";
const userName = (user?: EventUser | null) => `${user?.firstName ?? ""} ${user?.lastName ?? ""}`.trim();
const amount = (data: Record<string, unknown>) => {
  const cents = data.amountCents ?? data.refundedAmountCents ?? data.totalCents;
  if (typeof cents === "number") return `€${(cents / 100).toFixed(2)}`;
  if (typeof data.amount === "number") return `€${data.amount.toFixed(2)}`;
  return null;
};
const reviewIssuer = (event: EventRow, data: Record<string, unknown>) => {
  const reviewType = String(data.reviewType ?? "");
  const isClient = event.actorType === "CLIENT" || reviewType.includes("CLIENT_TO_PROVIDER");
  const isProvider = event.actorType === "PROVIDER" || reviewType.includes("PROVIDER_TO_CLIENT");
  if (isClient) return `${userName(event.client) || "Client"} (C)`;
  if (isProvider) return `${userName(event.provider) || "Provider"} (P)`;
  return null;
};
const eventDetail = (event: EventRow) => {
  const data = event.data ?? {};
  if (event.type === "STATUS_CHANGED") {
    // Older/backfilled events may not have retained `to`; the order snapshot
    // returned by the endpoint gives the best available resulting status.
    const nextStatus = data.to ?? data.newStatus ?? data.status ?? event.orderStatus;
    return nextStatus ? pretty(String(nextStatus)) : "Status updated";
  }
  if (event.type === "PROVIDER_PAYOUT" && data.ledgerType === "PROVIDER_TRANSFER_CREATED") return amount(data);
  if (event.type === "PROVIDER_PAYOUT" && data.ledgerType === "PROVIDER_PAYOUT_PAID") return amount(data);
  if (event.type === "PROVIDER_PAYOUT" && data.ledgerType === "PROVIDER_PAYOUT_FAILED") return amount(data);
  if (event.type.startsWith("PAYMENT_") || event.type.startsWith("REFUND_") || event.type === "PROVIDER_PAYOUT" || event.type === "CANCELLATION_FEE") return amount(data);
  if (event.type === "REVIEW_SUBMITTED" || event.type === "RATING_SUBMITTED") {
    const issuer = reviewIssuer(event, data);
    const rating = data.rating ?? data.rate ?? data.generalRating;
    const hasText = data.hasText === true || (typeof data.text === "string" && data.text.trim().length > 0);
    return [issuer, typeof rating === "number" ? `Rating: ${rating}/5` : null, hasText ? "Text review" : null].filter(Boolean).join(" · ") || null;
  }
  if (event.type.startsWith("REMINDER_")) {
    const reminder = data.reminderType ? pretty(String(data.reminderType)) : "Reminder";
    const recipient = data.recipientType ? pretty(String(data.recipientType)) : data.reminderType === "PROVIDER_SELECTION" ? "Client" : null;
    const sequence = typeof data.sequence === "number" ? `#${data.sequence}` : null;
    return [reminder, recipient ? `to ${recipient}` : null, sequence].filter(Boolean).join(" · ");
  }
  if (event.type === "PRICE_CHANGED" && Array.isArray(data.fields)) return `Changed: ${data.fields.map((field) => pretty(String(field))).join(", ")}`;
  return null;
};
const eventTitle = (event: EventRow) => {
  if (event.type === "REVIEW_SUBMITTED" && event.data?.hasText === false) return "Rating Submitted";
  if (event.type === "PROVIDER_PAYOUT" && event.data?.ledgerType === "PROVIDER_TRANSFER_CREATED") return "Provider Transfer Created";
  if (event.type === "PROVIDER_PAYOUT" && event.data?.ledgerType === "PROVIDER_PAYOUT_PAID") return "Provider Payout Paid";
  if (event.type === "PROVIDER_PAYOUT" && event.data?.ledgerType === "PROVIDER_PAYOUT_FAILED") return "Provider Payout Failed";
  return pretty(event.type);
};
const axiosErrorMessage = (error: unknown) => {
  if (typeof error === "object" && error !== null) {
    const response = (error as { response?: { data?: { error?: string } } }).response;
    if (response?.data?.error) return response.data.error;
  }
  return "Failed to load order events. The API endpoint may not be deployed yet.";
};

export default function OrderEventsPage() {
  const [items, setItems] = useState<EventRow[]>([]);
  const [page, setPage] = useState(1);
  const [total, setTotal] = useState(0);
  const [sort, setSort] = useState("time");
  const [orderPrettyId, setOrderPrettyId] = useState("");
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [eventTypes, setEventTypes] = useState<string[]>([]);
  const [eventTypesError, setEventTypesError] = useState("");
  const [selectedEvents, setSelectedEvents] = useState<string[]>([]);
  const [selectedStatuses, setSelectedStatuses] = useState<string[]>([]);
  const [isEventFilterOpen, setIsEventFilterOpen] = useState(false);
  const [isStatusSubmenuOpen, setIsStatusSubmenuOpen] = useState(false);
  const [isRegenerating, setIsRegenerating] = useState(false);
  const [isBackfilling, setIsBackfilling] = useState(false);
  const [backfillNotice, setBackfillNotice] = useState("");
  const [filterVersion, setFilterVersion] = useState(0);
  const eventFilterRef = useRef<HTMLDivElement>(null);
  const pageSize = 50;
  const load = async () => { setLoading(true); setError(""); try { const response = await getOrderEventsList({ page, pageSize, sort, orderPrettyId: orderPrettyId || undefined, eventNames: selectedEvents.length ? selectedEvents : undefined, statuses: selectedStatuses.length ? selectedStatuses : undefined, dateFrom: dateFrom ? new Date(`${dateFrom}T00:00:00`).toISOString() : undefined, dateTo: dateTo ? new Date(`${dateTo}T23:59:59.999`).toISOString() : undefined }); setItems(response.data.items ?? []); setTotal(Number(response.data.total ?? 0)); } catch (err) { setError(axiosErrorMessage(err)); } finally { setLoading(false); } };
  const loadEventTypes = async () => { setEventTypesError(""); try { const response = await getOrderEventTypes(); setEventTypes(response.data.types ?? []); } catch (err) { setEventTypesError(axiosErrorMessage(err)); } };
  useEffect(() => { void loadEventTypes(); }, []);
  useEffect(() => { void load(); }, [page, sort, filterVersion]);
  useEffect(() => { if (!isEventFilterOpen) return; const handler = (event: MouseEvent) => { if (eventFilterRef.current && !eventFilterRef.current.contains(event.target as Node)) { setIsEventFilterOpen(false); setIsStatusSubmenuOpen(false); } }; document.addEventListener("mousedown", handler); return () => document.removeEventListener("mousedown", handler); }, [isEventFilterOpen]);
  const toggleEvent = (type: string) => setSelectedEvents((prev) => (prev.includes(type) ? prev.filter((value) => value !== type) : [...prev, type]));
  const toggleStatus = (status: string) => setSelectedStatuses((prev) => (prev.includes(status) ? prev.filter((value) => value !== status) : [...prev, status]));
  const clearEventFilters = () => { setSelectedEvents([]); setSelectedStatuses([]); setIsStatusSubmenuOpen(false); };
  const regenerate = async () => { setIsRegenerating(true); setEventTypesError(""); try { await regenerateOrderEventTypes(); await loadEventTypes(); } catch (err) { setEventTypesError(axiosErrorMessage(err)); } finally { setIsRegenerating(false); } };
  const backfill = async () => { setIsBackfilling(true); setBackfillNotice(""); setError(""); try { const response = await backfillMissingCancellationEvents(); const result = response.data.result ?? {}; setBackfillNotice(`Done. Inserted ${result.inserted ?? 0} missing cancellation event${(result.inserted ?? 0) === 1 ? "" : "s"} (scanned ${result.scanned ?? 0}, already present ${result.existingByKey ?? 0}, covered by other event ${result.coveredByExistingEvent ?? 0}, failed ${result.failed ?? 0}).`); await load(); } catch (err) { setError(axiosErrorMessage(err)); } finally { setIsBackfilling(false); } };
  const submit = (event: FormEvent) => { event.preventDefault(); setIsEventFilterOpen(false); setIsStatusSubmenuOpen(false); setPage(1); setFilterVersion((version) => version + 1); };
  const sortedEventTypes = [...eventTypes].sort((a, b) => pretty(a).localeCompare(pretty(b)));
  const statuses = Object.values(OrderStatusEnum).sort((a, b) => pretty(a).localeCompare(pretty(b)));
  const eventFilterLabel = selectedEvents.length === 0 && selectedStatuses.length === 0 ? "Event name" : [selectedEvents.length ? `${selectedEvents.length} event${selectedEvents.length > 1 ? "s" : ""}` : null, selectedStatuses.length ? `${selectedStatuses.length} status${selectedStatuses.length > 1 ? "es" : ""}` : null].filter(Boolean).join(", ");
  return <ModalPageTemplate isScrollable><main className={styles.main}><div className={styles.header}><div><h1>Order events</h1><span>{total} events</span></div><form onSubmit={submit} className={styles.filters}><input placeholder="Order pretty ID" value={orderPrettyId} onChange={(e) => setOrderPrettyId(e.target.value)} /><div className={styles.eventFilter} ref={eventFilterRef}><button type="button" className={styles.eventFilterButton} onClick={() => { setIsEventFilterOpen((open) => !open); setIsStatusSubmenuOpen(false); }}><span>{eventFilterLabel}</span><span className={styles.eventFilterChevron}>{isEventFilterOpen ? "▲" : "▼"}</span></button>{isEventFilterOpen && <div className={styles.eventFilterPanel}><div className={styles.eventFilterList}>{eventTypesError ? <p className={styles.eventFilterEmpty}>{eventTypesError}</p> : sortedEventTypes.length === 0 ? <p className={styles.eventFilterEmpty}>Loading events…</p> : sortedEventTypes.map((type) => <div key={type} className={styles.eventFilterItem}><label className={styles.eventFilterRow}><input type="checkbox" checked={selectedEvents.includes(type)} onChange={() => toggleEvent(type)} /><span className={styles.eventFilterName}>{pretty(type)}</span>{type === "STATUS_CHANGED" && <button type="button" className={styles.eventFilterSubmenuToggle} title="Show resulting statuses" onClick={() => setIsStatusSubmenuOpen(true)}>›</button>}</label></div>)}</div>{isStatusSubmenuOpen && <div className={styles.statusSubmenu}><div className={styles.statusSubmenuHeader}><button type="button" className={styles.statusSubmenuBack} onClick={() => setIsStatusSubmenuOpen(false)}>← Events</button><span className={styles.statusSubmenuTitle}>Status changed</span></div><ul className={styles.statusSubmenuList}>{statuses.map((status) => <li key={status}><label className={styles.eventFilterRow}><input type="checkbox" checked={selectedStatuses.includes(status)} onChange={() => toggleStatus(status)} /><span className={styles.eventFilterName}>{pretty(status)}</span></label></li>)}</ul></div>}</div>}</div>{(selectedEvents.length > 0 || selectedStatuses.length > 0) && <button type="button" className={styles.eventFilterClear} onClick={clearEventFilters}>Clear</button>}<label>From <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} /></label><label>To <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} /></label><select value={sort} onChange={(e) => { setSort(e.target.value); setPage(1); }}><option value="time">Time</option><option value="event">Event</option><option value="order">Order</option></select><button type="submit">Search</button></form><button type="button" className={styles.regenerateButton} disabled={isRegenerating} onClick={() => void regenerate()}>{isRegenerating ? "Regenerating…" : "Regenerate events"}</button><button type="button" className={styles.regenerateButton} disabled={isBackfilling || isRegenerating} onClick={() => void backfill()}>{isBackfilling ? "Backfilling…" : "Backfill missing cancellation events"}</button>{backfillNotice && <span className={styles.backfillNotice}>{backfillNotice}</span>}</div>{error && <p className={styles.error}>{error}</p>}<section className={styles.list}>{loading ? <p>Loading...</p> : items.length === 0 ? <p>No events found.</p> : items.map((item) => { const detail = eventDetail(item); return <article className={styles.row} key={item.id}><div className={styles.orderCell}><div className={styles.avatarGroup}><div className={styles.avatar}>{item.provider?.imgUrl ? <img src={item.provider.imgUrl} alt="" /> : "O"}</div><div className={styles.avatar}>{item.client?.imgUrl ? <img src={item.client.imgUrl} alt="" /> : "C"}</div></div><div><a href={`/orders/${item.orderId}`}>{item.orderPrettyId ?? item.orderId}</a><small>{item.provider ? `${item.provider.firstName ?? ""} ${item.provider.lastName ?? ""}` : "Provider not selected"}</small></div></div><div className={styles.eventCell}><strong>{eventTitle(item)}</strong>{detail && <span className={styles.eventDetail}>{detail}</span>}</div><span className={styles.meta}>{item.actorType ?? "SYSTEM"} · {item.source ?? "API"}</span><time>{date(item.occurredAt)}</time></article>; })}</section><div className={styles.pagination}><button disabled={page <= 1 || loading} onClick={() => setPage((v) => v - 1)}>Previous</button><span>Page {page} of {Math.max(1, Math.ceil(total / pageSize))}</span><button disabled={page >= Math.ceil(total / pageSize) || loading} onClick={() => setPage((v) => v + 1)}>Next</button></div></main></ModalPageTemplate>;
}
